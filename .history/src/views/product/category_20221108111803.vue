...
const allowDrop = (draggingNode: Node, dropNode: Node, type: DropType) => {
  ...
  //当前拖动的节点+父节点所在深度不太于3
  let deep = maxLevel - draggingNode.data.catLevel + 1;
  if (type == "inner") {
  ...
  if (node.childNodes != null && node.childNodes.length > 0) {
    for (let i = 0; i < node.childNodes.length; i++) {
      if (node.childNodes[i].catLevel > maxLevel) {
        maxLevel = node.childNodes[i].catLevel;
      }
...
const handleDrop = async (
  ...
  //当前拖拽节点的最新层级
  await updateSort(updateNode);
  feedback.msgSuccess("更新节点成功");
  //刷新出新菜单
  getMenu();
  //设置需要默认展开的菜单
  expandedKey.value = [pCid];
};
//更改子节点层级
